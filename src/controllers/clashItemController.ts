...
const createClashItem = asyncHandler(async (req, res) => {
  ...
  if (!files) throw new ApiError(422, "2 files are required.");

  if (files.length != 2) throw new ApiError(422, "2 files are required.");

  const uploadResponseArray = await Promise.allSettled<UploadApiResponse>(
  ...
  const fullFilled =
    uploadResponseArray as PromiseFulfilledResult<UploadApiResponse>[];
...
const voteClashItem = asyncHandler(async (req, res) => {
  ...
  if (!clashItem) throw new ApiError(404, "Clash item not found.");
